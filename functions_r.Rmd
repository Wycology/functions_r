...
The bulk of the work here comes from the works of [@wickham2023]. Which should be pretty clear to majority of R users.

Virtually every time one uses `R`, a function is involved. It is not enough to use a function, it is worth to understand what the function does and even better still to tweak the function to suit your needs or even develop your own function(s).
...
I can be written using curly braces as follows:
...
square_root <- function(b){
  product <- b^0.5 # Square root of a number is the number power half
  return(product/2)
}
square_root(9)
```
...
Here we will create a function that will take a vector, calculate the mean, calculate the standard deviation, then divide deviations of values from the mean by the standard deviation
...
It is also possible to give a function default parameter(s) when defining it. This can be changed by the user when giving appropriate parameter to the argument to show results otherwise.
