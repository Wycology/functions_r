...
Here we are goin to create a function that will take a vector, calculate mean, calculate standard deviation, then divide deviations of values from mean by standard deviation
...
```
It is possible to give few unique initials of the name of the argument and the function will run successfully. For example, in our `rescale` function, the arguments start with x and o, so only stating the first name character is enough as follows:
...
```
It is clear from the above plot that the for loop spent less time iterate through the 1,000,000 length vector.
